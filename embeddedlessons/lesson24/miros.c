...
void OS_sched(void) {
	...
	++OS_currIdx;
	// Wraps index to 0 when it reaches the number of added threads
...
void OS_run(void) {
	...
	OS_onStartup(); // Will configure and enable interrupts
	...
	void *stkSto, uint32_t stkSize) 
{
	...
	*(--sp) = 0x00000004u; /* R4 */
	...
	Q_ASSERT(OS_threadsAdded < Q_DIM(OS_thread));
	// Q_ASSERT will call Q_onAssert() error handler if the expression inside evaluates to false
	OS_thread[OS_threadsAdded] = me; // adds current thread to array
	++OS_threadsAdded;
